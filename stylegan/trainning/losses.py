...
class LogisticGAN(GANLoss):
    ...
    # gradient penalty
    def R1Penalty(self, real_img, height, alpha, labels= None):
        ...
        real_logit = self.dis(real_img, height, alpha, labels)
    ...
    def dis_loss(self, real_samps, fake_samps, height, alpha, labels= None, r1_gamma=10.0):
        # Obtain predictions
        r_preds = self.dis(real_samps, height, alpha, labels)
        f_preds = self.dis(fake_samps, height, alpha, labels)
    ...
    def gen_loss(self, _, fake_samps, height, alpha, labels= None):
        f_preds = self.dis(fake_samps, height, alpha, labels)
