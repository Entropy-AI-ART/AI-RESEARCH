...
import trainning.losses as Losses
from trainning import update_average
from trainning.blocks import (DiscriminatorBlock, DiscriminatorTop,
                           GSynthesisBlock, InputBlock)
from trainning.layers import (EqualizedConv2d, EqualizedLinear,
...
class GMapping(nn.Module):
    ...
    def __init__(self, 
        ...
        if label_dim:
            self.embeding = nn.Linear(label_dim, latent_size, bias= False)
        ...
        layers.append(('dense0', EqualizedLinear(self.latent_size * 2 if label_dim else self.latent_size, self.mapping_fmaps,
    ...
    def forward(self, x, labels= None):
        if labels:
            embeded_label = self.embeding(labels)
            x = torch.cat([embeded_label, x], dim= 1)
...
class GSynthesis(nn.Module):
    ...
    def forward(self, dlatents_in, depth=0, alpha=0.0):
        """
...
class Generator(nn.Module):
    ...
    def __init__(self, 
        ...
        # if label_dim:
        #     assert label_dim > 0, "Conditional generation requires n_class > 0"
        #     self.class_embedding = nn.Embedding(label_dim, latent_size)
        #     latent_size *= 2
        ...
        self.g_mapping = GMapping(latent_size, label_dim, dlatent_size, dlatent_broadcast=self.num_layers, **kwargs)
    ...
    def forward(self, latents_in, depth, alpha, labels_in=None):
        ...
        dlatents_in = self.g_mapping(latents_in, labels_in= labels_in)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, 
        ...
        # if conditional:
        #     assert label_dim > 0, "Conditional Discriminator requires n_class > 0"
        #     self.embedding = nn.Embedding(label_dim, num_channels * resolution ** 2)
        #     num_channels *= 2
        #     self.embeddings = []
        ...
        self.final_block = DiscriminatorTop(self.mbstd_group_size, self.mbstd_num_features,
                                            ...
                                            gain=gain, use_wscale=use_wscale, activation_layer=act, 
                                            output_features= max(1, label_dim))
    ...
    def forward(self, images_in, depth, alpha=1., labels_in=None):
        ...
        if labels_in:
            scores_out = torch.sum(scores_out * labels_in, dim=1, keepdims=True)
        return scores_out
...
class StyleGAN:
    ...
    def optimize_discriminator(self, noise, real_batch, depth, alpha, labels=None):
        ...
        for _ in range(self.d_repeats):
            ...
            fake_samples = self.gen(noise, depth, alpha, labels_in= labels).detach()
    ...
    def train(self, 
        ...
        for current_depth in range(start_depth, self.depth):
            ...
            logger.info("Current resolution: %d x %d" % (current_res[0], current_res[0]))
