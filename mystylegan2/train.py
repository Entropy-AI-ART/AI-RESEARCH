...
if __name__ == '__main__':
    ...
    if opt.model.G.c_dim == 'auto':
        opt.model.G.c_dim = len(dataset.list_attributes)
    logger.info("Training with label: %d", opt.model.G.c_dim)
    # exit(0)
    ...
    if args.gen_shadow_file is not None and opt.stylegan2.use_ema:
