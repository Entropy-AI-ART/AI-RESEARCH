...
class StyleGAN:
    ...
    def optimize_generator(self, noise, real_batch, labels=None, gain= 1, reg= True):
        ...
        if reg:
            ...
            shirnk_labels = labels[:self.loss.pl_batch_shrink] if labels is not None else None
        ...
        if self.use_ema:
            ema_nimg = self.ema * 1000
            batch_size = noise.size(0)
            ema_beta = 0.5 ** (batch_size / max(ema_nimg, 1e-8))
            self.ema_updater(self.gen_shadow, self.gen, ema_beta)
            ...
            for i, batch in enumerate(data, 1):
                ...
                if i % int(total_batches / feedback_factor + 1) == 0 or i == 1:
                    ...
                    with torch.no_grad():                            
                        ...
                        # exit(0)
                        numpy_images = samples.add(-lo).mul(255 / (hi - lo)).clamp(0, 255).permute(0, 2, 3, 1).to('cpu').numpy().astype(np.uint8)
