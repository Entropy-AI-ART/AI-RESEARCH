...
class Stylegan2Loss(GANLoss):
    ...
    def R1Penalty(self, real_img, labels= None, reg= True):
        ...
        real_logit = self.dis(real_img, labels)
        if not reg:
            return real_logit
    ...
    def dis_loss(self, real_samps, fake_samps, labels= None, gain= 1, reg= True):
        ...
        lossDgen = torch.mean(nn.Softplus()(f_preds)) * gain
        lossDgen.backward()
        ...
        if self.r1_gamma != 0.0 and reg:
            real_logit, r1_penalty = self.R1Penalty(real_samps.detach(), reg= reg)
            (real_logit * 0 + lossDreal + r1_penalty * (self.r1_gamma / 2)).mean().mul(gain).backward()
        else:
            real_logit = self.R1Penalty(real_samps.detach(), reg= reg)
            (real_logit * 0 + lossDreal).mean().mul(gain).backward()
          
        
        return loss, r1_penalty.item() if reg else 0
    ...
    def gen_loss(self, _, fake_samps, shirnk_image_gen, gen_ws_skirnk, gain, labels= None, reg= True):
        ...
        loss_Gmain = torch.mean(nn.Softplus()(-f_preds)) * gain
        loss_Gmain.backward()
